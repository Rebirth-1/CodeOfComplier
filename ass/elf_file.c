...
{
	//-----elf头
	int *p_id = (int *)ehdr.e_ident;	//共16字节
	*p_id = 0x464c457f;		//前四字节固定: ELF 和 DEL控制符
	p_id++;
	*p_id = 0x010101;		//第五字节 文件类别:	0 无效文件; 1 32位ELF文件; 2 64位ELF文件
	p_id++;					//第六字节	 字节序:	0 无效格式; 1 小端字节序； 2 大端
	*p_id = 0;				//第七字节	ELF版本:	默认1
	p_id++;
	*p_id = 0;
	ehdr.e_type = ET_REL;
	...
	shstrtabSize = 51;		   //".rel.text".length()+".rel.data".length()+".bss".length()
							   ...
							   //注意每俩个字符串中间加一个空格所以最后需要加5
	char *str = shstrtab = new char[shstrtabSize];
	int index = 0;
	//段表串名与索引映射
	hash_map<string, int, string_hash> shstrIndex;
	shstrIndex[".rel.text"] = index;		//0
	strcpy(str + index, ".rel.text");
	shstrIndex[".text"] = index + 4;		//4
	index += 10;
	shstrIndex[""] = index - 1;				//9
	shstrIndex[".rel.data"] = index;		//10
	strcpy(str + index, ".rel.data");
	shstrIndex[".data"] = index + 4;		//14
	index += 10;
	shstrIndex[".bss"] = index;				//20
	strcpy(str + index, ".bss");
	index += 5;
	shstrIndex[".shstrtab"] = index;		//25
	strcpy(str + index, ".shstrtab");
	index += 10;
	shstrIndex[".symtab"] = index;			//35
	strcpy(str + index, ".symtab");
	index += 8;
	shstrIndex[".strtab"] = index;			//42
	...
	for (int i = 0; i < relTab.size(); i++)
	{
		...
		rel->r_info = ELF32_R_INFO((Elf32_Word)getSymIndex(relTab[i]->lbName), relTab[i]->type);//高24位表示符号表内的索引，低8为表示重定向类型
