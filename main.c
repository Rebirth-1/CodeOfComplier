...
bool genAsm = false, genObj = false, showLex = false, showSyn = false, showSem = false, showGen = false, showTab = false, showAss = false, showLink = false;
vector<string> srcFileNames;	//源文件目录列表
vector<string> kernelFileNames; //源文件核心文件名列表
string desFileName = "";		//输出文件
string curDir;					//当前程序绝对目录
int main(int argc, char *argv[])
{
	char buffer[256];
	curDir = getcwd(buffer, 256); //获取当前路径
	curDir += "/";
	if (argc == 1 || argc == 2 && strcmp(argv[1], "-h") == 0) //相当于./cit || ./cit -h
	{
	...
	else if (argc == 2 && strcmp(argv[1], "-v") == 0) // ./cit -v
	{
...
#define NEXT              \
	...
	NEXT while (flag) //先获取 ./cit 然后循环获取参数
	{
	...
	while (flag)
	{
		NEXT if (arg == "-s") genAsm = true;
		if (arg == "-o")
			genObj = true;
		if (arg == "-x")
			showLex = showSyn = showSem = showGen = showTab = showAss = showLink = true;
		if (arg == "-c")
			showLex = showSyn = showSem = showGen = showTab = true;
		if (arg == "-lex")
			showLex = true;
		if (arg == "-syn")
			showSyn = true;
...
void work()
{
	...
	for (int i = 0; i < kernelFileNames.size(); i++)
	{
		...
		if (access(realName.c_str(), F_OK) != 0) //检测文件是否存在
		{
			flag = false;
