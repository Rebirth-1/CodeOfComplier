...
void dec() // tested
{
  ...
  else if (token == rsv_extern) // TODO 外部变量声明  ??? 不包含函数 ???
  {
...
void funtail(symbol dec_type, string dec_name)
{
  ...
  else if (token == lbrac) //函数定义
  {
    ...
    table.addfun();
    BACK
        block(0, level, 0, 0); // TODO
    level = 0;                 //恢复
    tfun.poplocalvars(-1);     //清除参数
    genFuntail();
    return;
  }
  else if (token == ident || token == rsv_if || token == rsv_while || token == rsv_return || token == rsv_break || token == rsv_continue || token == rsv_in || token == rsv_out || token == rbrac) //必然是函数定义
  {
    BACK
        block(0, level, 0, 0);
    level = 0; //恢复
...
void varlist(symbol dec_type) //变量类似 int a, b, c
{
  if (token == comma)
  {
    nextToken();
...
{
  nextToken();
  if (token == semicon || token == rsv_while || token == rsv_if || token == rsv_return || token == ident || token == rsv_break || token == rsv_continue || token == rsv_in || token == rsv_out) //正常的语句
  {
...
void statement(int &var_num, int &level, int lopId, int blockAddr)
{
  ...
  case rsv_break:
    p("break语句", 0);
    nextToken();
    if (token == ident || token == rsv_while || token == rsv_if || token == rsv_return || token == rsv_break || token == rsv_continue || token == rsv_in || token == rsv_out || token == rbrac)
    {
      synterror(semiconlost, -1);
      BACK
    }
...
void ifstat(int &var_num, int &level, int lopId, int blockAddr)
{
  ...
  {
    if (token == lbrac ||
        token == semicon || token == rsv_while || token == rsv_if || token == rsv_return || token == rsv_break || token == rsv_continue || token == rsv_in || token == rsv_out || token == rsv_void || token == rsv_int || token == rsv_char || token == rsv_string) // else 丢失
    {
...
var_record *idtail(string refname, int &var_num)
{
  ...
  {
    identinexpr = 0;
    BACK return table.getVar(refname);
  }
  else
  {
