...
/*
	...
	symAddr:重定位符号的虚拟地址
*/
void SegList::relocAddr(unsigned int relAddr, unsigned char type, unsigned int symAddr)	//TODO
{
	...
	for (int i = 0; i < symDef.size(); ++i) //遍历定义的符号,寻找重定义信息，和入口信息
	{
		...
		for (int j = i + 1; j < symDef.size(); ++j) //遍历后边定义的符号
		{
				...
				)								   //类型相同[!!!不允许函数名符号和变量名符号相同,在汇编时无法区分这种差别!!!]
			{
				...
				// else type="符号";
				printf("符号名%s在文件%s和文件%s中发生链接冲突。\n", symDef[i]->name.c_str(), symDef[i]->prov->elf_dir, symDef[j]->prov->elf_dir);
				flag = false;
			}
		}
	}
	...
	for (int i = 0; i < segNames.size(); ++i) //按照类型分配地址，不紧邻.data与.bss段
	{
		...
		if (showLink)
			printf("%s\taddr=%08x\toff=%08x\tsize=%08x(%d)\n", segNames[i].c_str(),
				   segLists[segNames[i]]->baseAddr, segLists[segNames[i]]->offset, segLists[segNames[i]]->size, segLists[segNames[i]]->size);
	}
}
